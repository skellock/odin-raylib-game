...
init_reshuffler_test :: proc(t: ^testing.T) {
	reshuffler := main.init_reshuffler()
	defer main.destroy_reshuffler(&reshuffler)

	testing.expect_value(t, reshuffler.cooldown.duration, main.RESHUFFLE_COOLDOWN)
...
reshuffler_cooldown_test :: proc(t: ^testing.T) {
	...
	main.start_timer(&reshuffler.cooldown)
	testing.expect_value(t, reshuffler.cooldown.active, true)

	// after partial time, still active
	main.update_timer(&reshuffler.cooldown, main.RESHUFFLE_COOLDOWN * 0.5)
	testing.expect_value(t, reshuffler.cooldown.active, true)

	// after full duration, cooldown expires
	main.update_timer(&reshuffler.cooldown, main.RESHUFFLE_COOLDOWN * 1.5)
	testing.expect_value(t, reshuffler.cooldown.active, false)
}
...
reshuffler_cooldown_resets_test :: proc(t: ^testing.T) {
	...
	main.start_timer(&reshuffler.cooldown)
	main.update_timer(&reshuffler.cooldown, main.RESHUFFLE_COOLDOWN)
	testing.expect_value(t, reshuffler.cooldown.active, false)

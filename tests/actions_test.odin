package tests

import main ".."
import "core:testing"

@(test)
resolve_actions_reshuffle_test :: proc(t: ^testing.T) {
	actions: main.Actions
	input: main.Input
	g := main.init_game()
	defer main.destroy_game(&g)

	input.mouse.right_pressed = false
	g.reshuffler.cooldown.active = false
	main.resolve_actions(&actions, g, input)
	testing.expect_value(t, actions.reshuffle, false)

	input.mouse.right_pressed = true
	g.reshuffler.cooldown.active = false
	main.resolve_actions(&actions, g, input)
	testing.expect_value(t, actions.reshuffle, true)

	input.mouse.right_pressed = true
	g.reshuffler.cooldown.active = true
	main.resolve_actions(&actions, g, input)
	testing.expect_value(t, actions.reshuffle, false)

	input.mouse.right_pressed = false
	g.reshuffler.cooldown.active = true
	main.resolve_actions(&actions, g, input)
	testing.expect_value(t, actions.reshuffle, false)
}

@(test)
resolve_actions_move_dot_test :: proc(t: ^testing.T) {
	actions: main.Actions
	input: main.Input
	g := main.init_game()
	defer main.destroy_game(&g)

	input.mouse.left_pressed = false
	main.resolve_actions(&actions, g, input)
	testing.expect_value(t, actions.move_dot.active, false)

	input.mouse.left_pressed = true
	input.mouse.world_pos = {1.1, 2.2}
	main.resolve_actions(&actions, g, input)
	testing.expect_value(t, actions.move_dot, main.MoveDotAction{true, {1.1, 2.2}})
}

@(test)
resolve_actions_quit_game_test :: proc(t: ^testing.T) {
	actions: main.Actions
	input: main.Input
	g := main.init_game()
	defer main.destroy_game(&g)

	input.keyboard.quit_pressed = false
	main.resolve_actions(&actions, g, input)
	testing.expect_value(t, actions.quit_game, false)

	input.keyboard.quit_pressed = true
	main.resolve_actions(&actions, g, input)
	testing.expect_value(t, actions.quit_game, true)
}

@(test)
resolve_actions_toggle_pause_test :: proc(t: ^testing.T) {
	actions: main.Actions
	input: main.Input
	g := main.init_game()
	defer main.destroy_game(&g)

	input.keyboard.pause_pressed = false
	main.resolve_actions(&actions, g, input)
	testing.expect_value(t, actions.toggle_pause, false)

	input.keyboard.pause_pressed = true
	main.resolve_actions(&actions, g, input)
	testing.expect_value(t, actions.toggle_pause, true)
}

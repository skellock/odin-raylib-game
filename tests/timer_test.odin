...
timer_init_test :: proc(t: ^testing.T) {
	...
	expect_value(t, timer.duration, f32(1.0))
	expect_value(t, timer.elapsed, f32(0.0))
	expect_value(t, timer.active, false)
	expect_value(t, timer.one_shot, false)
}
...
timer_init_one_shot_test :: proc(t: ^testing.T) {
	...
	expect_value(t, timer.one_shot, true)
}
...
timer_update_active_test :: proc(t: ^testing.T) {
	...
	expect_value(t, timer.elapsed, f32(0.3))
...
timer_update_one_shot_stops_test :: proc(t: ^testing.T) {
	...
	expect_value(t, timer.active, false)
	expect_value(t, timer.elapsed, f32(1.0))
}

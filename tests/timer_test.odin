...
init_timer_one_shot_test :: proc(t: ^testing.T) {
	timer := main.init_timer(2.0, one_shot = true)
...
one_shot_stops_test :: proc(t: ^testing.T) {
	timer := main.init_timer(1.0, one_shot = true)
	timer.active = true
	main.update_timer(&timer, 1.5)
	testing.expect_value(t, timer.active, false)
